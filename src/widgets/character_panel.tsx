...
function CharacterPanel() {
  ...
  const questLog: QuestLogState = questLogRaw ?? {
    dailyDate: '',
    weeklyKey: '',
    dailies: [],
    weeklies: [],
  ...
  async function buyItem(shop: ShopItem) {
    if (!rawChar) return;
  ...
  return (
    <div
      style={{
        ...
        color: '#f5f5f5',
        background: 'radial-gradient(circle at top, #2b2b3a 0, #050509 60%)',
    ...
    >
      ...
      >
        <img
          src={`${rootURL}ui/sidebar_icon.png`}
          alt=""
          style={{ width: '28px', height: '28px', imageRendering: 'pixelated', borderRadius: '4px' }}
          onError={(e) => {
            (e.target as HTMLImageElement).style.display = 'none';
          }}
        />
        <div>
          <div style={{ fontSize: '14px', fontWeight: 700, color: '#c79c6e', textShadow: '0 0 4px #000' }}>
            ⚔️ World of Remcraft
          </div>
      ...
      <div style={{ padding: '10px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
          <img
            ...
            alt="Charakter"
            style={{ width: '64px', height: '64px', imageRendering: 'pixelated', borderRadius: '6px', border: '1px solid #705030' }}
            onError={(e) => {
              (e.target as HTMLImageElement).outerHTML = '<span style="font-size:32px">🧙</span>';
            }}
          />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '13px', fontWeight: 700, color: '#c79c6e' }}>
              Level {level}{level >= 60 ? ' (MAX)' : ''}
            </div>
            {character.streakDays > 1 && (
              <div style={{ fontSize: '10px', color: '#fb923c' }}>🔥 Streak: {character.streakDays} days</div>
            )}
            <div style={{ fontSize: '10px', color: '#b0b0c0' }}>
              {character.cardsAnswered} cards · {character.totalXP} XP
            </div>
            ...
            <div style={{ fontSize: '9px', color: '#888', marginTop: '1px' }}>
              {level < 60 ? `${character.currentXP} / ${xpNeeded} XP` : 'Max level reached!'}
            </div>
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <>
            <div style={{ border: '1px solid #705030', borderRadius: '8px', padding: '8px', background: '#1b1b27', marginBottom: '8px', boxShadow: '0 0 8px rgba(0,0,0,0.6)' }}>
              <div style={{ textAlign: 'center', marginBottom: '4px' }}>
                {enemy.imageKey ? (
                  <img
                    ...
                    alt={enemy.name}
                    style={{ width: '64px', height: '64px', imageRendering: 'pixelated', borderRadius: '4px' }}
                    onError={(e) => {
              ...
              <div style={{ fontWeight: 700, textAlign: 'center', marginBottom: '4px', fontSize: '12px', color: enemy.elite ? '#ff5555' : '#ffd27f', textShadow: '0 0 4px #000' }}>
                {enemy.name}
              </div>
              ...
              <div style={{ fontSize: '10px', color: '#c0c0c0', marginTop: '2px', textAlign: 'center' }}>
                ❤️ {Math.max(0, enemy.currentHP)} / {enemy.maxHP} · ⚔️ {enemy.damage} Dmg/round
              </div>
            </div>
            ...
            <div style={{ marginBottom: '8px', padding: '6px 8px', background: '#0d0d1a', borderRadius: '6px', border: '1px solid #463218' }}>
              ...
              {milestones.map((m, i) => (
                <div key={i} style={{ fontSize: '9px', color: '#aaa', marginBottom: '1px' }}>{m}</div>
              ))}
            </div>

            <div style={{ padding: '8px', background: 'linear-gradient(180deg, #20150a 0%, #120c06 100%)', borderRadius: '8px', border: '1px solid #7c5a26', marginBottom: '8px', boxShadow: 'inset 0 0 0 1px rgba(255,190,90,0.08), 0 0 10px rgba(0,0,0,0.35)' }}>
              <div style={{ fontSize: '11px', color: '#ffd27f', fontWeight: 700, marginBottom: '6px' }}>Active Daily Quest</div>
              {!dailyQuest ? (
                <div style={{ fontSize: '9px', color: '#888' }}>No daily quest loaded yet.</div>
              ) : (
                <div>
                  <div style={{ fontSize: '10px', color: dailyQuest.completed ? '#86efac' : '#f3e2b8', fontWeight: 700 }}>
                    {dailyQuest.completed ? '✅ ' : '❓ '}{dailyQuest.title}
                  </div>
            ...
            <div style={{ padding: '8px', background: 'linear-gradient(180deg, #1d1308 0%, #100905 100%)', borderRadius: '8px', border: '1px solid #6b4920', marginBottom: '8px', boxShadow: 'inset 0 0 0 1px rgba(255,190,90,0.06), 0 0 10px rgba(0,0,0,0.3)' }}>
              <div style={{ fontSize: '11px', color: '#fdba74', fontWeight: 700, marginBottom: '6px' }}>Active Weekly Quest</div>
              {!weeklyQuest ? (
                <div style={{ fontSize: '9px', color: '#888' }}>No weekly quest loaded yet.</div>
              ) : (
                <div>
                  <div style={{ fontSize: '10px', color: weeklyQuest.completed ? '#86efac' : '#f3e2b8', fontWeight: 700 }}>
                    {weeklyQuest.completed ? '✅ ' : '❓ '}{weeklyQuest.title}
                  </div>
        ...
        {activeTab === 'shop' && (
          <div style={{ padding: '8px', background: '#0d1a0d', borderRadius: '8px', border: '1px solid #2d5a1b', marginBottom: '8px' }}>
            ...
            <div style={{ fontSize: '9px', color: '#9ca3af', marginBottom: '8px' }}>Buy healing or scrolls without cluttering the main overview.</div>
            {SHOP_ITEMS.map((shop) => {
              ...
              return (
                <div key={shop.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <button
                    onClick={() => buyItem(shop)}
                    disabled={!canAfford}
                    style={{ fontSize: '9px', padding: '4px 8px', borderRadius: '4px', border: 'none', cursor: canAfford ? 'pointer' : 'not-allowed', background: canAfford ? '#15803d' : '#333', color: canAfford ? '#fff' : '#666', minWidth: '58px' }}
                  >
                    {costLabel}
                  </button>
                </div>
        ...
        {activeTab === 'stats' && (
          <div style={{ padding: '8px', background: '#111', borderRadius: '8px', border: '1px solid #333', marginBottom: '8px' }}>
            ...
            <div style={{ fontSize: '9px', color: '#94a3b8', marginBottom: '8px' }}>All long-term values in one place without overloading the main view.</div>
            <div style={{ display: 'grid', gap: '6px' }}>
              <div style={{ padding: '6px 8px', background: '#171717', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                <div style={{ fontSize: '9px', color: '#888' }}>🧟 Enemies defeated</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#f3f4f6' }}>{character.monstersDefeated}</div>
              </div>
              <div style={{ padding: '6px 8px', background: '#171717', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                <div style={{ fontSize: '9px', color: '#888' }}>🏆 Elites defeated</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#fbbf24' }}>{character.elitesDefeated}</div>
              </div>
              <div style={{ padding: '6px 8px', background: '#171717', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                <div style={{ fontSize: '9px', color: '#888' }}>📚 Cards answered</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#93c5fd' }}>{character.cardsAnswered}</div>
              </div>
              <div style={{ padding: '6px 8px', background: '#171717', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                <div style={{ fontSize: '9px', color: '#888' }}>✨ Total XP</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#c4b5fd' }}>{character.totalXP.toLocaleString()}</div>
              </div>
              <div style={{ padding: '6px 8px', background: '#171717', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                <div style={{ fontSize: '9px', color: '#888' }}>🔥 Current streak</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#fb923c' }}>{character.streakDays} days</div>
              </div>
            </div>
          </div>
        )}
        ...
        <button
          onClick={handleReset}
          style={{ width: '100%', padding: '6px', borderRadius: '5px', border: 'none', cursor: 'pointer', fontSize: '10px', fontWeight: 600, background: confirmReset ? '#7f1d1d' : '#1a0000', color: confirmReset ? '#fca5a5' : '#666', transition: 'all 0.2s' }}
        >
